...
app.registerExtension({
  name: "AKXZRangeLora.dynamicInputs",

  async beforeRegisterNodeDef(nodeType, nodeData, app) {
    log("beforeRegisterNodeDef", nodeData?.name);

    if (nodeData?.name !== "AK XZ Batch Lora") return;
    ...
    function pairConnected(node, i) {
      const a = getInput(node, `model_${i}`);
      const b = getInput(node, `con_${i}`);
    ...
    function ensurePairExists(node, i) {
      ...
      const modelType = getInput(node, "model_0")?.type || "MODEL";
      const conType = getInput(node, "con_0")?.type || "CONDITIONING";

      if (!a) {
        node.addInput(`model_${i}`, modelType);
        log("addInput", `model_${i}`, modelType);
      }
      if (!b) {
        node.addInput(`con_${i}`, conType);
        log("addInput", `con_${i}`, conType);
      }
    }
    ...
    function removeTrailingUnconnectedPairs(node, minKeepPairs) {
      ...
      for (let i = 10; i >= minKeepPairs; i--) {
        const aIdx = findInputIndex(node, `model_${i}`);
        const bIdx = findInputIndex(node, `con_${i}`);
